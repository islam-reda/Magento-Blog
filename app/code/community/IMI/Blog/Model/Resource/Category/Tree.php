 ...
 */
class IMI_Blog_Model_Resource_Category_Tree extends Varien_Data_Tree_Dbp
{
     ...
     *
     * @var IMI_Blog_Model_Resource_Category_Collection
     */
    ...
    public function __construct()
    {
        ...
        parent::__construct(
            $resource->getConnection('imi_blog_write'),
            $resource->getTableName('imi_blog/category'),
            array(
     ...
     *
     * @access public
     * @param IMI_Blog_Model_Resource_Category_Collection $collection
    ...
    {
        $collection = Mage::getModel('imi_blog/category')->getCollection();
        if ($sorted) {
            if (is_string($sorted)) {
    ...
    {
        Mage::getResourceSingleton('imi_blog/category')
    ...
    {
        Mage::app()->cleanCache(array(IMI_Blog_Model_Category::CACHE_TAG));
        return $this;
    }
    ...
    protected function _createCollectionDataSelect($sorted = true)
    {
        ...
        $categoriesTable = Mage::getSingleton('core/resource')
            ->getTableName('imi_blog/category');
    ...
    protected function _getInactiveItemIds($collection)
    {
        ...
        $table = Mage::getSingleton('core/resource')->getTable('imi_blog/category');
        $bind = array(
            'cond' => 0,
        );
        $select = $this->_conn->select()
